...
#ifndef OMPSS_ONLY_SMP
#pragma omp target device(fpga) num_instances(1) \
	 copy_inout([localLength]wv) copy_in([localLength]vv)
#endif
#pragma omp task in([localLength]vv) inout([localLength]wv)
void copy_vector_fpga(local_int_t localLength, double *vv, double *wv) {
...
void CopyVector_nw(const Vector & v, Vector & w) {
  ...
  double * wv = w.values;
	...
	copy_vector_fpga(localLength,vv,wv);
#pragma omp taskwait noflush
	return;
}
